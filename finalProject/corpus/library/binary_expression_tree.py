...
class BinaryExpressionTree(object):
    ...
    def insert(self, expression: str):
        ...
        while len(stack) != 0:
            ...
            if '.' in char or char.isdigit() or char.isalpha() :
    ...
    def evaluate(self, node=None) -> string:
        """
    ...
    def infix_to_postfix(self, infix_input: list) -> list:
        ...
        precedence_order = {'+': 0, '-': 0, '*': 1, '/': 1, '^': 2,'=':3}
        associativity = {'+': "LR", '-': "LR", '*': "LR", '/': "LR", '^': "RL",'=':"LR"}
...
if __name__ == "__main__":
    ...
    exmpl = "(x1 + y)*(a + q)/5 + 1 * 0"
    
    # ignore the script and grab the user expression
    # user_input = sys.argv[1:]
    tree_obj = BinaryExpressionTree('((((a+b)/3)-(3+f))=4)') ## 2*4 + 2*a
